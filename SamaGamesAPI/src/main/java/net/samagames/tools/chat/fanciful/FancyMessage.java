...
import net.samagames.tools.ArrayWrapper;
import org.bukkit.*;
...
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	 ...
	 * @return This builder instance.
	 */
	public FancyMessage achievementTooltip(@SuppressWarnings("deprecation") final Achievement which) {
		try {
	...
	*/
	...
	private Object createChatPacket(String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		if(nmsChatSerializerGsonInstance == null){
			...
			double majorVersion = Double.parseDouble(split[0].substring(1, 2)) + Double.parseDouble(split[1]) / Math.pow(10, split[1].length());
			int lesserVersion = Integer.parseInt(split[2].substring(1, 2));

			if (majorVersion > 1.2 && (majorVersion < 1.8 || (majorVersion == 1.8 && lesserVersion == 1))) { //This api will never run on 1.1 / 1.2, so i can use this check to run 1.10
